Sono presenti quattro notebook: uno per effettuare il fine tuning,uno per generare le risposte con il modello con il fine tuning, uno per effettuare la valutazione per span e uno per generare le risposte con zero e fewshot ed estrarle correttamente.
...
Tramite il notebook della valutazione invece è possibile generare la valutazione sul test set tramite span con il tipo: è necessario solamente passare alle due funzioni i path del file del dataset e quello dei risultati e le funzioni si occuperanno di calcolare le metriche

Il notebook dello zero/fewshot si differenzia da quello classico per la presenza di diversi prompt e per alcune modifiche alla funzione per estrarre le entita dalla prediction, in modo da gestire casi che non si verificano con altri modelli.
